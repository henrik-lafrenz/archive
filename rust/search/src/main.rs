                    ...
                    Ok(mut zipped_file) => {
                        if zipped_file.name().ends_with("info.txt") {
                            ...
                            zipped_file
                                ...
                                .expect("couldn't read zipped file");

                            found = Some(info_text);
                            break;
                        }
                    }
                    Err(e) => println!("couldn't get zipped file in {:?}: {:?}", zip_path, e),
                }
            }
        }
        Err(e) => println!(
            "couldn't instantiate zip object for {:?}: {:?}",
            zip_path, e
        ),
    }
...
fn collect_search_results(
    ...
    search_str: &String,
) -> Option<Vec<data_types::SearchResult>> {
    ...
    loop {
        ...
        match result {
            Some(info_text) => {
                ...
                if found.is_some() {
                    search_results.push(data_types::SearchResult {
                        info_text: info_text,
